...
function App() {
  ...
  async function loadProjects() {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/v1/video-projects?workspaceId=${encodeURIComponent(scopedWorkspaceId)}`);
  ...
  async function createVideo() {
    ...
    if (!scopedWorkspaceId) return setError('Inserisci il Workspace ID.');
    if (!trimmedPrompt) return setError('Scrivi l’idea del video.');
    setBusy(true);
    setArtifacts([]);
    try {
      const createResponse = await fetch(`${API_BASE}/api/v1/video-projects`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ workspaceId: scopedWorkspaceId, channelId: null, prompt: trimmedPrompt })
      });
      ...
      setError(cause instanceof Error ? cause.message : 'Impossibile avviare la produzione.');
    } finally {
      setBusy(false);
    }
  }
      ...
      <div className="mx-auto flex min-h-screen max-w-[1440px]">
        <aside className="hidden w-60 shrink-0 border-r border-zinc-800/80 px-4 py-6 lg:block">
          ...
          <nav className="mt-8 space-y-1">
            {navigation.map(([label, Icon], index) => <button key={label} onClick={() => label === 'Create Video' && setShowCreate(true)} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm ${index === 0 ? 'bg-zinc-800 text-white' : 'text-zinc-500 hover:bg-zinc-900 hover:text-zinc-200'}`}><Icon size={17} />{label}</button>)}
          </nav>
        </aside>
