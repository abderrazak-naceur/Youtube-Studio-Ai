...
export function ResearchWorkspacePanel({ apiBase, workspaceId }: Props) {
  ...
  const opportunityById = useMemo(() => new Map(opportunities.map(opportunity => [opportunity.id, opportunity.title])), [opportunities]);
  const availableOpportunities = useMemo(() => opportunities.filter(opportunity => !projects.some(project => project.opportunityId === opportunity.id)), [opportunities, projects]);
  ...
  async function loadProjects() {
    ...
    try {
      const [projectsResponse, opportunitiesResponse] = await Promise.all([
        fetch(`${apiBase}/api/v1/research-projects?workspaceId=${encodeURIComponent(scopedWorkspaceId)}`),
        fetch(`${apiBase}/api/v1/opportunities?workspaceId=${encodeURIComponent(scopedWorkspaceId)}&sort=opportunityScore&direction=desc`)
      ]);
    ...
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'Unable to load research projects.'); }
  }

  async function loadSources(projectId: string) {
    if (!workspaceId.trim() || !projectId) { setSources([]); return; }
    try {
    ...
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'Unable to load research sources.'); }
  }
  ...
  async function createProject() {
    if (!workspaceId.trim()) return setError('Workspace ID is required.');
    if (!selectedOpportunityId) return setError('Choose an opportunity before starting research.');
    setBusy(true); setError('');
    try {
      const response = await fetch(`${apiBase}/api/v1/research-projects`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ workspaceId: workspaceId.trim(), opportunityId: selectedOpportunityId }) });
      if (!response.ok) throw new Error(await response.text());
      const project: ResearchProject = await response.json();
      setSelectedOpportunityId('');
      await loadProjects();
      setSelectedProjectId(project.id);
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'Unable to start research.'); }
    finally { setBusy(false); }
  }

  async function addSource() {
    if (!workspaceId.trim() || !selectedProjectId || !title.trim() || !url.trim()) return setError('Workspace, source title and URL are required.');
    setBusy(true); setError('');
    try {
      const response = await fetch(`${apiBase}/api/v1/research-projects/${selectedProjectId}/sources`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ workspaceId: workspaceId.trim(), title: title.trim(), url: url.trim(), metadataJson: metadataJson.trim() || null }) });
      if (!response.ok) throw new Error(await response.text());
      setTitle(''); setUrl(''); setMetadataJson(''); await loadSources(selectedProjectId);
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'Unable to add research source.'); }
    finally { setBusy(false); }
  }
